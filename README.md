...
Python から SQL Server へ接続するためのドライバです。
...
**最初のアクセスでテーブル `dbo.todos` が作成されます。** 2 回目以降は作成しません。
...
Azure VM（Ubuntu 24.04 LTS）にアプリを配置して公開します。**ローカル実行手順の手順 4 と手順 5（Azure SQL Database の作成、接続情報の設定）を先に終わらせてください。**
...
`/opt/todo/src/.env` に、ローカル実行手順の手順 5 と同じ内容を記入します。
...
**原因** — Azure SQL Database のサーバーレス構成は、無操作が続くと自動的に一時停止します。**停止中のデータベースへ接続すると、再開が終わるまで待たされます。** 仕組みは[設計書の「接続の再試行」](docs/design.md#接続の再試行)を参照してください。
...
| 5   | テーブルの定義が古い             | 上の 4 つに当てはまらない場合                    | 次の「テーブルを作り直したい」を参照します      |
...
**クエリ エディターにもファイアウォール規則が必要です。** 必要なのは**ブラウザを動かしている開発 PC の IP アドレス**で、ローカル実行手順の手順 4 で登録したものです（VM の送信元 IP ではありません）。変わっている場合は、エラーの中の「Allowlist IP ...」を選ぶとその場で追加できます。

続けてアプリを再起動します。VM の場合です。

```bash
sudo systemctl restart todo
```

ローカルの場合は `Ctrl + C` で停止して、起動し直します。
