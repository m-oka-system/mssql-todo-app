...
**80 番はそのままにします。** 理由は[設計書の「経路ごとの仕様」](docs/design.md#経路ごとの仕様)を参照してください。
...
**画面の下端には実行環境の情報が出ます。** 応答した VM のホスト名とプライベート IP、接続先のデータベース名を表示します。仕様は[設計書の「実行環境の表示」](docs/design.md#実行環境の表示)を参照してください。
...
**セクションが終わったら、Azure ポータルでリソースグループごと削除してください。** 環境を残すとデータベースの無料枠を消費し続けます。

**`terraform destroy` は使いません。** Terraform はリソースグループ自体を管理しておらず、作成したものはすべてその中にあります。**リソースグループを消せば確実に片付きます。**

**Cloud Shell に残るファイルを消す必要はありません。** `terraform apply` は接続情報と秘密鍵、`terraform.tfstate` を実行した端末に作りますが、**ストレージなし（エフェメラル）で使うため、セッションが切れるとまとめて消えます。**
...
**Terraform で構築した環境では使えません。** **VM 上に** `~/mssql-todo-app` が残らないためです（取得したリポジトリを一時ディレクトリへ置いて削除します）。**リソースグループごと削除して、作り直してください。**
