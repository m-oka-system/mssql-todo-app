...
| ポート | 用途                   | 許可する送信元          |
| ------ | ---------------------- | ----------------------- |
| 22     | Cloud Shell からの SSH | **Cloud Shell の IP**   |
| 80     | ブラウザからのアクセス | **ブラウザの端末の IP** |

Cloud Shell の IP を確認します。

```bash
curl -s https://api.ipify.org
```

ブラウザの IP は、<https://api.ipify.org> を新しいタブで開くと分かります。**2 つは別のアドレスです。**

Azure ポータルで VM の「ネットワーク設定」を開き、受信規則をそれぞれ編集して、ソースに上で調べた IP アドレスを設定します。
