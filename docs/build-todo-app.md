...
**同じ環境を Terraform で自動構築する[コース内ラボ](../labs/sections/section04-web-server/01-create-todo-environment/README.md)があります。** ただし**コース内ラボ環境（Udemy のワークスペース）での実施が前提**で、ご自身のサブスクリプションで実行すると利用料金が発生します。ここでの手作業の手順と、ラボが自動化する範囲の対応は、ラボ側にまとめています。
...
**Cloud Shell へアップロードした秘密鍵も消してください。**

```bash
rm -f ~/.ssh/ssh-key.pem
```

**VM を削除しても鍵は残ります。** ストレージなし（エフェメラル）で使っている場合はセッションとともに消えますが、マウントしている場合はファイル共有に残り続けます。
