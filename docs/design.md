...
| エディタ（VM） | micro（`.env` の修正に使用）         | 2.0.13     | apt（universe）             |
...
**経路は 2 つありますが、`/opt/todo/src/.env` へ置く処理は 1 か所です。** どちらも `deploy/setup.sh --env-file` を通り、所有者を `todo`、パーミッションを 600 にします。

| 環境の作り方   | `setup.sh` へ渡すまで                                |
| -------------- | ---------------------------------------------------- |
| ポータルで手動 | Cloud Shell で作り、`scp` で VM のホームへ送ります   |
| Terraform      | 拡張機能が一時ファイルへ書き出し、そのパスを渡します |

**受講者が配置先・所有者・パーミッションを指定する場面はありません。** `--env-file` にパスを渡すだけで、`install -o todo -g todo -m 600` 相当をスクリプトが行います。**サービスを起動する前に配置するため、再起動も要りません。**

**値そのものは引数に渡しません。** コマンドラインに書くと `ps` の出力とシェルの履歴に接続情報が残ります。渡すのはファイルのパスだけです。

**手動の経路では、編集を Cloud Shell 側で行います。** VM 上のエディタで打ち直すより速く、打ち間違いも起きません。**接続情報は Cloud Shell の `~/.env` と VM のホームに残る**ため、どちらも[構築手順](build-todo-app.md)の中で削除します。
...
**Cloud Shell をストレージなし（エフェメラル）で起動した場合は、セッションが切れるとまとめて消えます。** ストレージをマウントしている場合と、作成者が手元で実行した場合は残ります。削除の手順は[ラボの片付け](../labs/sections/section99/README.md#片付け)にあります。

**この経路ではアプリの取得と起動も同時に行います。** 手順は[コース内ラボの「Todo アプリの環境を Terraform で構築する」](../labs/sections/section99/README.md)にあります。

**`.env` はリポジトリに含めません。** `.gitignore` の対象のため、`setup.sh` は `.env.sample` を雛形として配置するだけです。

### VM へ取得する範囲

**VM では `git clone --depth 1` でリポジトリ全体を取得します。** 絞り込みは行いません。

**`--depth 1` の目的は転送量ではありません。** リポジトリは 1 MB 未満で、履歴を含めても速度は変わりません。**最初の取得時に過去の版を持ち込まないことが目的です。**

**`--sparse` は使いません。** 以前は `src/` と `deploy/` だけを展開していましたが、割に合わないため外しました。

| 期待していた効果 | 実際                                                           |
| ---------------- | -------------------------------------------------------------- |
| 通信量が減る     | 減りません。`sparse-checkout` が絞るのは展開範囲だけです       |
| 配置内容が変わる | 変わりません。`deploy/setup.sh` はコピーする対象を名指しします |
| ディスクが減る   | 344 KB です。OS ディスクは 30 GB のため意味がありません        |

**代わりに `git sparse-checkout set src deploy` の 1 手順が増えていました。** 得られるのは `~/mssql-todo-app` を `ls` したときの見た目だけです。

**`/opt/todo` へ配置されるのは、アプリの実行に必要なファイルだけです。** `deploy/setup.sh` が `src/` と直下 3 ファイル（`pyproject.toml`・`uv.lock`・`.python-version`）を名指しでコピーするため、`docs/` や `labs/` が作業ツリーにあっても配置対象には入りません。
