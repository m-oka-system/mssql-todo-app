...
flowchart LR
    ...
    P[開発 PC] -->|TDS 1433 / TLS| D
```

**開発 PC と VM は同じデータベースに接続します。** データベースは 1 つだけ作ります。
...
| エラーの案内 | `error.html` | 接続失敗またはテーブル未作成のときの案内 |

一覧の 1 件は、完了ボタン・タスク名・作成日時・削除ボタンで構成します。完了済みのタスク名には取り消し線を引きます。

## 4. ルーティング仕様

**このアプリは JSON を返しません。** すべて HTML を返すか、リダイレクトします。利用者はブラウザで、フォームの送信とリダイレクトで操作します。
...
| 503    | データベースへ接続できない、またはテーブルが存在しない |
...
`src/schema.sql` の内容です。**管理者が一度だけ実行します。**
...
**アプリはテーブルを作成しません。** 複数の VM やワーカーが同時に起動すると、作成処理が競合するためです。テーブルが存在しない場合は、実行手順を画面に案内します。
...
| 用途           | SQL                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------ |
| 存在確認       | `SELECT OBJECT_ID(N'dbo.todos', N'U')`                                                     |
...
アプリの 1 接続あたりの最悪値は 70 秒です（試行 10 秒 × 5 回 + 待機 5 秒 × 4 回）。**一覧の表示はテーブルの存在確認と一覧の取得で接続を最大 2 回張るため、1 リクエストでは 140 秒を見込みます。**
...
| テーブルが存在しない       | `schema.sql` の実行を案内 | 503  |
