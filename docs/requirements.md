...
**無料枠は 1 つのデータベースにつき月 100,000 vCore 秒です。** 停止しないまま**約 1.7 日**で使い切ります。使い切ったデータベースは翌月まで再開しませんが、**料金は発生せず、作り直せば新しい無料枠のデータベースになります。** 上限はデータベースごとに数えるためです。

**セクションごとに環境を作り直す使い方であれば、この状態には至りません。** 至った場合も、リソースグループごと削除してラボを実行し直せば復旧します。

**無料オファーのデータベースは 1 サブスクリプションにつき 10 個までです。** 削除した分の枠が空くまで最大 1 時間かかるため、短時間に何度も作り直すと 11 個目で失敗します。
